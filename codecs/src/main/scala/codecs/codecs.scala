...
trait EncoderInstances {
  ...
  implicit val stringEncoder: Encoder[String] =
    Encoder.fromFunction(s => Json.Str(s.toString)) // TODO Implement the `Encoder[String]` instance
...
trait DecoderInstances {
  ...
  implicit val IntDecoder: Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(n) => if((n).isValidInt) n.toInt else  -2}
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(n) =>  Some(n.flatMap(decoder.decode))
    }

  // (n.map(decoder.decode)
  /**
    * A decoder for JSON objects. It decodes the value of a field of
    * the supplied `name` using the given `decoder`.
    */
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] = Decoder.fromFunction {
    case Json.Obj(n) => decoder.decode(n(name))
  }
